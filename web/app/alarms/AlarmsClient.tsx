...
export function AlarmsClient() {
  ...
  const accountRow = (a: AlarmAccount, alarm_type: AlarmType) => {
    ...
    // 이동선은 계좌가 저장할 기준이 없다 — 이평선 일수는 종목이 속한 종목풀 설정에서 온다.
    const currentValues: Record<string, number> =
      isMa || a.stoploss_threshold_pct === null ? {} : { threshold_pct: a.stoploss_threshold_pct };
        ...
        <div className="card appCard">
          <div className="card-body">
            ...
            <p style={{ color: "var(--text-muted)", fontSize: "var(--fs-sm)", marginBottom: 12 }}>
              계좌별로 켜고 끕니다. 판정 기준(단기·장기 이평선)은 <strong>종목이 속한 종목풀</strong> 설정을 씁니다.
              보유 종목의 종가가 둘 중 하나라도 아래면 알립니다(종목풀 순위 화면의 회색 처리와 같은 기준).
            </p>
