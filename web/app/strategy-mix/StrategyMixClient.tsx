...
const CURRENT_NOTES = [
  ...
  },
  {
    ...
    body:
      ...
      "목표 계산에는 엔진 기준일 가격을 사용하며, 장중 표시 시세와 분리합니다.",
  },
  {
    title: "목표 주수",
    body:
      "조회할 때마다 계좌 보유를 기준일 가격으로 평가하고 현재 현금을 더한 계좌 평가액으로 목표 금액을 환산합니다. " +
      "매매 대상이 아닌 고정 자산은 배분에서 제외합니다. 중복 종목의 목표 금액을 합친 뒤 주수를 내림하고, " +
      "내림으로 남은 예산은 소수부가 큰 종목부터 최대 한 주씩 추가 배정합니다. 다음 종목을 살 수 없으면 멈춥니다. " +
      "합성 유보 현금과 전략 내부 현금은 추가 배정에 쓰지 않습니다. 기존 보유가 내림보다 한 주 많다는 이유로 유지하지 않습니다. " +
      "기준일 가격·계좌 평가액이 바뀌면 내림이나 추가 배정 결과가 달라져 소액 조정이 생길 수 있습니다.",
  },
  {
    title: "액션",
    body:
...
export function StrategyMixClient() {
  ...
  const positionRows = useMemo<PositionRow[]>(() => {
    ...
    const cashRow: PositionRow = {
      ...
      // 실제목표 현금 — 단주로 못 채운 몫까지 포함한다. 이 값이 있어야 실제목표 컬럼의
      // 합이 100% 가 된다(종목 실제목표 합 + 현금).
      actual_weight_pct: positions.summary.actual_cash_pct,
                  ...
                  <div>
                    <div style={{ fontWeight: 700, marginBottom: 6 }}>
                      ...
                      <span style={{ ...hintStyle, marginLeft: 8, fontWeight: 500 }}>
                        ...
                        최소 금액 미만의 단순 조정은 제외합니다 · 교체 확정분은 교체일 그룹
                      </span>
                    </div>
                    {!hasActions ? (
                      <div style={hintStyle}>
