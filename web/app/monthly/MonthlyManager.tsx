...
type MonthlyGridRow = MonthlyRow & {
  id: string;
};
...
const COLUMN_DEFS = [
  { key: "month_date_display", label: "종료일" },
  { key: "memo", label: "비고" },
  ...
  { key: "monthly_profit", label: "금월 손익" },
  { key: "monthly_return_pct", label: "월수익률" },
...
function formatCellValue(row: MonthlyRow, key: (typeof COLUMN_DEFS)[number]["key"]): string {
  const value = row[key as keyof MonthlyRow];
  if (key === "month_date_display" || key === "memo") {
    return String(value ?? "-");
  }
...
function getColumnCellClass(key: string, value: number | string): string {
  ...
  if (typeof value === "number" && (key === "profit_loss" || key === "cumulative_profit" || key === "monthly_profit" || PERCENT_KEYS.has(key))) {
...
function parseMonthlyCellValue(field: MonthlyEditableField | undefined, newValue: unknown, oldValue: unknown) {
  if (!field) {
    return newValue;
  }
  if (field.type === "text") {
...
function getMonthlyColumnWidth(key: (typeof COLUMN_DEFS)[number]["key"]): { width?: number; minWidth: number; flex?: number } {
  if (key === "month_date_display") {
...
export function MonthlyManager({
  ...
  onHeaderSummaryChange?: (summary: { activeMonthDate: string; rowCount: number; dirtyCount: number }) => void;
}) {
  const [rows, setRows] = useState<MonthlyRow[]>([]);
  const [editableFields, setEditableFields] = useState<MonthlyEditableField[]>([]);
  ...
  async function load(options?: { silent?: boolean }) {
    ...
    try {
      const response = await fetch("/api/monthly", { cache: "no-store" });
      const payload = (await response.json()) as MonthlyResponse;
      if (!response.ok) {
        throw new Error(payload.error ?? "월별 데이터를 불러오지 못했습니다.");
      }
      ...
      setActiveMonthDate(payload.active_month_date ?? "");
      setDirtyRowIds([]);
      setDirtyCellKeys([]);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : "월별 데이터를 불러오지 못했습니다.");
    } finally {
      if (!silent) {
        setLoading(false);
      }
  ...
  const gridRows = useMemo<MonthlyGridRow[]>(
    () =>
      rows.map((row) => ({
        ...row,
        id: row.month_date,
  ...
  }, [activeMonthDate, dirtyRowIds.length, onHeaderSummaryChange, rows.length]);
  ...
  const gridColumns = useMemo<ColDef<MonthlyGridRow>[]>(
    () => [
      ...visibleColumns.map<ColDef<MonthlyGridRow>>((column) => ({
        ...getMonthlyColumnWidth(column.key),
        field: column.key,
        headerName: column.label,
        pinned: column.key === "month_date_display" ? "left" : undefined,
        type:
        ...
        cellRenderer: (params: { data?: MonthlyGridRow; value?: unknown }) => (
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        setError(null);
        const dirtyRows = rows.filter((row) => dirtyRowIds.includes(row.month_date));
            ...
            body: JSON.stringify({
              ...
              ...Object.fromEntries(
                editableFields
                  ...
                  .map((field) => [field.key, row[field.key as keyof MonthlyRow]]),
              ),
            }),
          });
          const payload = (await response.json()) as { error?: string };
          if (!response.ok) {
            throw new Error(payload.error ?? "월별 데이터 저장에 실패했습니다.");
          }
        }
        await load({ silent: true });
        toast.success("[자산-월별] 변경사항 저장 완료");
      } catch (saveError) {
        setError(saveError instanceof Error ? saveError.message : "월별 데이터 저장에 실패했습니다.");
      }
    });
  }
        ...
        <div className="card appCard appTableCardFill">
              ...
              <div className="appMainHeader">
                <div className="appMainHeaderLeft weeklyMainHeaderLeft">
                  <label className="appLabeledField">
                    ...
                    <div className="appSegmentedToggle" role="group" aria-label="월별 보기 방식">
                      <button
                        type="button"
                  ...
                  <label className="appLabeledField">
                    ...
                    <span className="form-control form-control-sm bg-light text-secondary d-flex align-items-center">
                      평일 09:05~15:50 매 15분 일별 원장 기준 월별 자동 집계
                    </span>
                  </label>
                </div>
              </div>
          ...
          <div className="card-body appCardBodyTight appTableCardBodyFill">
            <AppAgGrid<MonthlyGridRow>
              ...
              getRowClass={(params) => (params.data?.month_date === activeMonthDate ? "tableRowSelected" : "")}
              gridOptions={{
                ...
                onCellValueChanged: (params: {
                  ...
                  setRows((current) =>
                    current.map((row) =>
                      row.month_date === rowId
                        ? {
                            ...row,
                            [field]: params.data?.[field as keyof MonthlyGridRow] ?? row[field as keyof MonthlyRow],
                          }
                        : row,
                    ),
                  );
