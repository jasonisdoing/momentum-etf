...
const CURRENT_NOTES = [
  ...
  },
  {
    title: "청산",
    body:
      "자격(장기 이격 > 0, 단기 이격 ≥ 0)을 잃은 날이 매도 신호이고, 다음 거래일 시가에 팝니다. " +
      "진입 시 자산의 1/슬롯을 배정하고 보유 중 비중은 건드리지 않습니다 — 판 자리는 다음 진입까지 현금입니다.",
  },
  {
    title: "ADR 게이트",
    body:
      "ADR 하한을 설정하면, 그날 시장 ADR(20일 상승/하락 종목수 비율 — 시장은 종목풀 설정의 시장 레짐 지수)이 " +
      "하한 미만인 날은 신규 진입만 건너뜁니다. 보유·청산은 그대로 돌아갑니다. 없음이면 게이트를 쓰지 않습니다.",
  },
  {
    title: "장중 판정",
    body:
      "장중에는 실시간 가격을 마지막 봉으로 쓴 같은 신호 계산으로 판정을 보여줍니다. " +
      "규칙은 같고 입력만 잠정이라, 종가가 확정되면 백테스트와 일치합니다.",
  },
  {
    title: "표시",
...
export function MomentumClient() {
  ...
  const planRows = useMemo<PlanRow[]>(() => {
    ...
    const exited: PlanRow[] = positions.exited_today.map((trade) => ({
      ...
      listing_months: trade.listing_months,
    }));
    ...
    <PageFrame title="모멘텀 전략" fullWidth>
      <div className="appPageStack">
        ...
        <StrategyTuning
          ...
          fixedLabel={`저장된 설정 기준 (종목풀 ${draftPool} · 종목 수 ${view.settings.top_n} 공통 고정 · 일간 슬롯 · 교체 없음)`}
          current={{
