...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await fetchFastApiJson<AusMarketStocksResponse>(
      `/internal/aus-market-stocks/index?index=ASX200&min_market_cap_ukm=${encodeURIComponent(minMarketCapUkm)}`,
    );
    return jsonNoStore(data);
  } catch (error) {
    return jsonNoStore(
      { error: error instanceof Error ? error.message : "호주 개별주 데이터를 불러오지 못했습니다." },
      { status: 500 },
    );
  }
}
