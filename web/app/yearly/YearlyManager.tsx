...
const COLUMN_DEFS = [
  { key: "year_date_display", label: "종료일" },
  { key: "memo", label: "비고" },
  ...
  { key: "yearly_profit", label: "금년 손익" },
  { key: "yearly_return_pct", label: "연수익률" },
...
const yearlyGridTheme = createAppGridTheme();
...
function formatCellValue(row: YearlyRow, key: (typeof COLUMN_DEFS)[number]["key"]): string {
  const value = row[key as keyof YearlyRow];
  if (key === "year_date_display" || key === "memo") {
    return String(value ?? "-");
  }
...
function getColumnCellClass(key: string, value: number | string): string {
  ...
  if (typeof value === "number" && (key === "profit_loss" || key === "cumulative_profit" || key === "yearly_profit" || PERCENT_KEYS.has(key))) {
...
function parseYearlyCellValue(field: YearlyEditableField | undefined, newValue: unknown, oldValue: unknown) {
  if (!field) {
    return newValue;
  }
  if (field.type === "text") {
...
function getYearlyColumnWidth(key: (typeof COLUMN_DEFS)[number]["key"]): { width?: number; minWidth: number; flex?: number } {
  if (key === "year_date_display") {
...
export function YearlyManager({
  onHeaderSummaryChange,
}: {
  onHeaderSummaryChange?: (summary: { activeYearDate: string; rowCount: number; dirtyCount: number }) => void;
}) {
  const [rows, setRows] = useState<YearlyRow[]>([]);
  const [editableFields, setEditableFields] = useState<YearlyEditableField[]>([]);
  const [readOnlyKeys, setReadOnlyKeys] = useState<Set<string>>(new Set());
  const [activeYearDate, setActiveYearDate] = useState("");
  ...
  async function load(options?: { silent?: boolean }) {
    ...
    try {
      const response = await fetch("/api/yearly", { cache: "no-store" });
      const payload = (await response.json()) as YearlyResponse;
      if (!response.ok) {
        throw new Error(payload.error ?? "년별 데이터를 불러오지 못했습니다.");
      }
      ...
      setActiveYearDate(payload.active_year_date ?? "");
      setDirtyRowIds([]);
      setDirtyCellKeys([]);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : "년별 데이터를 불러오지 못했습니다.");
    } finally {
      if (!silent) {
        setLoading(false);
      }
  ...
  const gridRows = useMemo<YearlyGridRow[]>(
    () =>
      rows.map((row) => ({
        ...row,
        id: row.year_date,
  ...
  useEffect(() => {
    onHeaderSummaryChange?.({
      activeYearDate: activeYearDate || "-",
      rowCount: rows.length,
      dirtyCount: dirtyRowIds.length,
    });
  }, [activeYearDate, dirtyRowIds.length, onHeaderSummaryChange, rows.length]);
  ...
  const gridColumns = useMemo<ColDef<YearlyGridRow>[]>(
    () => [
      ...visibleColumns.map<ColDef<YearlyGridRow>>((column) => ({
        ...getYearlyColumnWidth(column.key),
        field: column.key,
        headerName: column.label,
        pinned: column.key === "year_date_display" ? "left" : undefined,
        type:
        ...
        cellRenderer: (params: { data?: YearlyGridRow; value?: unknown }) => (
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        setError(null);
        const dirtyRows = rows.filter((row) => dirtyRowIds.includes(row.year_date));
            ...
            body: JSON.stringify({
              ...
              ...Object.fromEntries(
                editableFields
                  ...
                  .map((field) => [field.key, row[field.key as keyof YearlyRow]]),
              ),
            }),
          });
          const payload = (await response.json()) as { error?: string };
          if (!response.ok) {
            throw new Error(payload.error ?? "년별 데이터 저장에 실패했습니다.");
          }
        }
        await load({ silent: true });
        toast.success("[자산-년별] 변경사항 저장 완료");
      } catch (saveError) {
        setError(saveError instanceof Error ? saveError.message : "년별 데이터 저장에 실패했습니다.");
      }
    });
  }
      ...
      <section className="appSection appSectionFill">
        <div className="card appCard appTableCardFill">
              ...
              <div className="appMainHeader">
                <div className="appMainHeaderLeft weeklyMainHeaderLeft">
                  <label className="appLabeledField">
                    ...
                    <div className="appSegmentedToggle" role="group" aria-label="년별 보기 방식">
                      <button
                        type="button"
                  ...
                  <label className="appLabeledField">
                    ...
                    <span className="form-control form-control-sm bg-light text-secondary d-flex align-items-center">
                      평일 09:35, 16:35 일별 원장 기준 년별 자동 집계
                    </span>
                  </label>
                </div>
              </div>
          ...
          <div className="card-body appCardBodyTight appTableCardBodyFill">
            <AppAgGrid<YearlyGridRow>
              rowData={gridRows}
              columnDefs={gridColumns}
              loading={loading || isPending}
              minHeight="100%"
              className="weeklyAgGrid"
              theme={yearlyGridTheme}
              getRowClass={(params) => (params.data?.year_date === activeYearDate ? "tableRowSelected" : "")}
              gridOptions={{
                  ...
                  setRows((current) =>
                    current.map((row) =>
                      row.year_date === rowId
                        ? {
                            ...row,
                            [field]: params.data?.[field as keyof YearlyGridRow] ?? row[field as keyof YearlyRow],
                          }
                        : row,
                    ),
                  );
