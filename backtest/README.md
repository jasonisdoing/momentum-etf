...
- `backtest/results/<pool>-backtest_details_<YYYY-MM-DD>.log`
  ...
  - 각 거래일 표의 첫 row는 항상 CASH이며, 당일 SELL 종목도 함께 남긴다. 상태값은 CASH, HOLD, BUY, SELL, WAIT 로 표기된다. (WAIT은 현재 보유 중이 아닌 종목 중 해당 신호일 추세(%) 상위 `TOP_N_HOLD` 후보를 참고용으로 보여준다.)
  - 상세 표에는 `추세(%)(D-1)`만 기록되며, 값은 해당 거래를 결정한 신호일 기준이다.
