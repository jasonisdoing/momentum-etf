...
- `TREND_WEIGHT_RATIO` (추세 가중치 %, 예 `[50,60,70,80]`) — 보유 제외 나머지 비중 중 추세 몫. **추세 = `(100−보유)×r/100`, 샤프 = `(100−보유)×(100−r)/100`**.
- `MA_TYPE`
- `MA_MONTHS`
- `RSI_LIMIT` (`us`, `kor` 종목풀만 사용)

> 점수식: `점수 = w_trend×추세(±100) + w_sec×샤프(±100) + 보유가점`, `보유가점 = 보유 시 보유%`. 라이브(`utils/rankings.py`)와 동일하다. 보조지표는 SHARPE 로 고정.
...
- `backtest/results/<pool>-backtest_details_<YYYY-MM-DD>.log`
  ...
  - `3. 월별 내역` 섹션에서 월별 수익률을 `전략`, `벤치마크`로 비교한다.
...
- `HOLDING_BONUS_SCORE`(보유보너스 %)는 가중치 탐색값이며, 최적 조합의 보유%는 백테스트 종료 시 라이브 `pool_settings`에 자동 저장된다(추세·샤프 비중은 보유%에서 유도되므로 별도 저장 없음)
