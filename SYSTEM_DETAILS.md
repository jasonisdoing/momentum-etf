...
- **슬리피지**:
  ...
  - 호주: 매수 +1.0%, 매도 -1.0%
  - 미국: 매수 +0.3%, 매도 -0.3%
- **리밸런싱**: 균등 비중 리밸런싱 (매일 체크)
  - 조건: 매수/매도 발생 OR 비중 편차 > `REBALANCE_THRESHOLD`
  - 목표: 각 종목 `100% / PORTFOLIO_TOPN` 균등 비중 유지
  - 방법: 과다 보유 종목 일부 매도 → 과소 보유 종목 추가 매수
...
| **슬리피지** | 있음 (0.5~1.0%) | 없음 | 없음 |
| **리밸런싱** | 균등 비중 (매일 체크) | 균등 비중 (매일 체크) | 없음 |
| **목적** | 전략 검증 | 실제 성과 | 비교 기준 |

> **참고**: 추천 시스템은 리밸런싱 로직이 없습니다. 리밸런싱은 백테스트와 퍼포먼스 계산에서만 사용되며, 실제 리밸런싱 실행은 별도 시스템에서 처리합니다.
...
| **REBALANCE_THRESHOLD** | 리밸런싱 임계값 (비중 편차 %) | 0.1~1.0% |
