...
# 가격 캐시 점검 슬랙
# ─────────────────────────────────────────────────────────────────────────────
# 외부 소스에도 없어 못 채운 결측(종가·거래량)을 **며칠 전 것까지** 슬랙으로 알릴지.
#
# 못 채운 결측은 다음 실행에서도 그대로 남아, 조건 없이 알리면 같은 내용이 매일 온다.
# (거래량 결측은 20일 평균 창에서 빠질 때까지 3~4주가 걸린다.) 발견 직후가 다시 시도해
# 볼 수 있는 시점이고 그 뒤로는 손쓸 방법이 없어서, 최근 것만 알리고 나머지는 로그에만
