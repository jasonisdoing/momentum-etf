"""
'coin' 국가, 'b1' 계좌에 대한 파라미터 튜닝을 실행하는 스크립트입니다.

이 스크립트는 `test.py`를 병렬로 실행하여
지정된 파라미터 범위 내에서 최적의 조합(최고 CAGR 기준)을 찾습니다.

[사용법]
python scripts/tune_coin_b1.py
...
- 고정할 값: 단일 값을 포함한 리스트로 지정 (예: `[5]`)

결과는 콘솔에 출력되고 `logs/tune_coin_b1.log` 파일에도 저장됩니다.
"""
...
TUNING_PARAMS = {
    "ma_period": np.arange(2, 5, 1),  # 10부터 200까지 10씩 증가
    "portfolio_topn": np.arange(1, 4, 1),
    "replace_threshold": [0],
}
# 백테스트 기간 (개월)
TEST_MONTHS_RANGE = 3
...
def main():
    """'coin' 국가, 'b1' 계좌에 대한 파라미터 튜닝을 실행합니다."""
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    log_dir = os.path.join(project_root, "logs")
    os.makedirs(log_dir, exist_ok=True)
    log_path = os.path.join(log_dir, "tune_coin_b1.log")
