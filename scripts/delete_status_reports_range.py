...
def main():
    ...
    if args.dry_run:
        ...
        print(
            f"[DRY-RUN] Would delete {count} status_reports for {args.country} from {args.start} to {args.end}"
        )
        return

    res = db.status_reports.delete_many(query)
    print(
        f"[RESULT] Deleted {res.deleted_count} status_reports for {args.country} from {args.start} to {args.end}"
    )
