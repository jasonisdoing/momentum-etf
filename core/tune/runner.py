...
@dataclass
class TuningSearchContext:
    """튜닝 탐색 공간과 기간 설정 묶음입니다."""
    ...
    warmup_days: int
    tune_months_value: int


@dataclass
class TuningPreparedContext:
    """튜닝 실행 직전까지 준비된 데이터와 출력 경로입니다."""

    txt_path: Path
    tuning_metadata: dict[str, Any]
    prefetched_map: dict[str, DataFrame]
    prefetched_trading_days: list[pd.Timestamp]
    prefetched_metrics_map: dict[str, dict[str, Any]]
...
def _resolve_tuning_search_context(
    *,
    account_id: str,
    ...
    """튜닝 탐색 공간과 기간 설정을 구성합니다."""

    common_cfg: dict[str, Any] = dict(config.get("COMMON") or {}) if isinstance(config.get("COMMON"), dict) else {}
    ...
    account_norm = account_id.strip().lower()
    ...
    if not etf_universe:
        raise RuntimeError(f"[튜닝] '{account_norm}' 종목 데이터를 찾을 수 없습니다.")
    ...
    if not valid_start_dates:
        raise RuntimeError("[튜닝] 유효한 기간 정보가 없습니다.")
    ...
    for start_date_str in valid_start_dates:
        try:
            item_start_dt = pd.to_datetime(start_date_str).normalize()
        except Exception:
            continue
        test_period_ranges.append(
            {
                "backtest_start_date": start_date_str,
                "start_date": item_start_dt.strftime("%Y-%m-%d"),
                "end_date": end_date.strftime("%Y-%m-%d"),
            }
        )
    if not test_period_ranges:
        raise RuntimeError("[튜닝] 유효한 테스트 기간이 없습니다.")
    ...
    try:
        ...
        ma_max = (
            max(ma_month_values) * 20 if ma_month_values else max([base_ma, *ma_values] if ma_values else [base_ma])
        )
    except ValueError:
        ma_max = 20
    ...
    return TuningSearchContext(
        ...
        tune_months_value=tune_months_value,
    )


def _prepare_tuning_execution_context(
    *,
    account_id: str,
    search_context: TuningSearchContext,
    output_path: Path | str | None,
    results_dir: Path | str | None,
    debug_dir: Path | None,
    logger,
) -> TuningPreparedContext:
    """프리패치 데이터와 출력 메타데이터를 준비합니다."""

    earliest_start = search_context.earliest_start
    date_range_prefetch = [
        earliest_start.strftime("%Y-%m-%d"),
        search_context.end_date.strftime("%Y-%m-%d"),
    ]
    ...
    if SAVE_CACHE_DURING_TUNE:
        for ticker, frame in prefetched_map.items():
            save_cached_frame(search_context.country_code, ticker, frame)

    if debug_dir is not None:
        _export_prefetched_data(debug_dir, prefetched_map)

    if results_dir is not None:
        base_dir = Path(results_dir) / get_account_dir(account_id).name / "results"
    else:
        base_dir = DEFAULT_RESULTS_DIR / get_account_dir(account_id).name / "results"
    ...
    tuning_metadata = {
        ...
        "country_code": search_context.country_code,
        "search_space": {
        ...
        "ticker_count": len(search_context.tickers),
        "excluded_tickers": [],
        "test_periods": search_context.valid_start_dates,
        "test_period_ranges": search_context.test_period_ranges,
    }

    debug_month_configs: list[dict[str, Any]] = []
    normalized_month_items: list[dict[str, Any]] = []
    for item in search_context.month_items:
        backtest_start_date = item.get("backtest_start_date")
        if not backtest_start_date:
            logger.warning("[튜닝] %s 백테스트 시작일이 없습니다. 항목을 건너뜁니다.", account_id.upper())
            continue
        try:
            pd.to_datetime(backtest_start_date)
        except Exception:
            logger.warning(
    ...
    prefetched_trading_days = get_trading_days(
        date_range_prefetch[0],
        date_range_prefetch[1],
        search_context.country_code,
    )
    if not prefetched_trading_days:
        raise RuntimeError(
            f"[튜닝] {account_id.upper()} 기간 {date_range_prefetch[0]}~{date_range_prefetch[1]}의 거래일 정보를 로드하지 못했습니다."
        )

    ma_days_pool = sorted(set(m * TRADING_DAYS_PER_MONTH for m in search_context.ma_month_values))
    ma_type_pool = sorted(set(search_context.ma_type_values))
    logger.info(
    ...
    return TuningPreparedContext(
        txt_path=txt_path,
        tuning_metadata=tuning_metadata,
        prefetched_map=prefetched_map,
        prefetched_trading_days=list(prefetched_trading_days),
        prefetched_metrics_map=prefetched_metrics_map,
        normalized_month_items=normalized_month_items,
        debug_month_configs=debug_month_configs,
    )
...
def _execute_tuning(
    ...
    *,
    ...
    if not ma_candidates or not ma_type_candidates or not rebalance_mode_candidates:
        logger.warning(
...
def run_account_tuning(
    ...
    *,
    ...
    search_context = _resolve_tuning_search_context(
        account_id=account_id,
        config=config,
    ...
    ma_count = len(search_context.ma_month_values)
    ma_type_count = len(search_context.ma_type_values)
    rebalance_count = len(search_context.rebalance_mode_values)

    logger.info(
        "[튜닝] 탐색 공간: MA %d개 × MA_TYPE %d개 × 리밸런스 %d개 = %d개 조합 (최적화 지표: %s)",
        ma_count,
        ma_type_count,
        rebalance_count,
        search_context.combo_count,
        search_context.optimization_metric,
    )
    prepared_context = _prepare_tuning_execution_context(
    ...
    debug_month_configs = prepared_context.debug_month_configs
    if not prepared_context.normalized_month_items:
        logger.warning("[튜닝] 실행 가능한 기간 항목이 없습니다.")
        return None

    # 최적화 지표에 따른 정렬 함수 정의
    optimization_metric_raw = search_context.search_space.get("OPTIMIZATION_METRIC")
    if isinstance(optimization_metric_raw, list):
        optimization_metric = optimization_metric_raw[0].upper()
    else:
        optimization_metric = str(optimization_metric_raw).upper()
    ...
    for idx, item in enumerate(prepared_context.normalized_month_items, 1):
        ...
        def save_progress_callback(success_entries, progress_pct, completed, total):
            ...
            _save_intermediate_results(
                ...
                tuning_metadata=prepared_context.tuning_metadata,
            )

        single_result = _execute_tuning(
            account_norm,
            backtest_start_date=backtest_start_date,
            search_space=search_context.search_space,
            end_date=search_context.end_date,
            excluded_tickers=set(),
            prefetched_data=prepared_context.prefetched_map,
            output_path=prepared_context.txt_path,
            progress_callback=save_progress_callback,
            prefetched_etf_universe=search_context.etf_universe,
            prefetched_metrics=prepared_context.prefetched_metrics_map,
            trading_calendar=prepared_context.prefetched_trading_days,
        )
        ...
        if results_per_month:
            _save_intermediate_results(
                ...
                progress_info={
                    ...
                    "total": len(prepared_context.normalized_month_items),
                },
                tuning_metadata=prepared_context.tuning_metadata,
            )
            logger.info(
                ...
                len(prepared_context.normalized_month_items),
            )

        if debug_dir is not None and capture_top_n > 0:
            raw_rows = single_result.get("raw_data") or []
            month_start = backtest_start_date
            month_end = search_context.end_date.strftime("%Y-%m-%d")
            calendar_for_month = _filter_trading_days(prepared_context.prefetched_trading_days, month_start, month_end)
    ...
    if runtime_missing_registry:
        unseen_missing = sorted(runtime_missing_registry)
        if unseen_missing:
            logger.warning(
    ...
    if results_per_month:
        ...
        best = item.get("best") or {}
        logger.info(
            "[튜닝] %s (%s 시작) 최적 조합 (%s 기준): REBALANCE_MODE=%s / CAGR=%.2f%% / Sharpe=%.2f / SDR=%.3f",
            account_norm.upper(),
    ...
    entry["result"]["TUNE_MONTHS"] = int(search_context.tune_months_value)
    ...
    try:
        if prepared_context.txt_path.exists():
            prepared_context.txt_path.unlink()
    except OSError:
        pass

    prepared_context.txt_path.write_text(
        "\n".join(report_lines) + "\n",
        encoding="utf-8",
    )
    ...
    logger.info("튜닝 요약을 '%s'에 기록했습니다.", prepared_context.txt_path)
    ...
    return prepared_context.txt_path
