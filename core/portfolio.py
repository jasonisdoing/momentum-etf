...
def portfolio_topn_series(
    ...
    (logic.a.logic.portfolio_topn_series에서 이동)
    """
    # Fetch all series
    data = {}
    for tkr, _ in pairs:
    ...
    if initial_positions and len(common_index) > 0:
        dt0 = common_index[0]
        for tkr, pos in initial_positions.items():
            if tkr in data and dt0 in data[tkr]['close'].index:
                price0 = float(data[tkr]['close'].loc[dt0])
                sh0 = int(pos.get('shares', 0) or 0)
                if sh0 > 0:
                    ac0 = pos.get('avg_cost')
                    state[tkr]['shares'] = sh0
                    state[tkr]['avg_cost'] = float(ac0) if ac0 and ac0 > 0 else price0
                    cash -= sh0 * price0
