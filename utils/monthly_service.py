...
FIELD_DEFS = [
    ...
    {"key": "monthly_profit", "label": "금월 손익", "type": "int"},
    {"key": "monthly_return_pct", "label": "월수익률 (%)", "type": "float"},
...
def _format_month_date_display(date_str: str) -> str:
    weekday_kr = ["월", "화", "수", "목", "금", "토", "일"]
    dt = datetime.datetime.strptime(date_str, "%Y-%m-%d")
    return f"{dt.year}. {dt.month}. {dt.day} ({weekday_kr[dt.weekday()]})"


def _get_month_range(year: int, month: int) -> tuple[datetime.date, datetime.date]:
    """해당 월의 첫날과 마지막 날을 반환한다."""
    first_day = datetime.date(year, month, 1)
    if month == 12:
        next_month = datetime.date(year + 1, 1, 1)
    else:
        next_month = datetime.date(year, month + 1, 1)
    return first_day, next_month - datetime.timedelta(days=1)
...
def _apply_running_total_principal(docs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """수익률 계산 규칙: monthly_return_pct = TWR(1월), cumulative_return_pct = ROI.
    ...
    for month_date in sorted(docs_by_date):
        ...
        if twr_base > 0:
            doc["monthly_return_pct"] = round((_to_int(doc.get("monthly_profit", 0)) / twr_base) * 100, 2)
        else:
...
def _doc_to_api_row(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "month_date": str(computed_doc["month_date"]),
        "month_date_display": _format_month_date_display(str(computed_doc["month_date"])),
        ...
        "monthly_profit": _to_int(computed_doc.get("monthly_profit", 0)),
        "monthly_return_pct": round(float(computed_doc.get("monthly_return_pct", 0.0) or 0.0), 2),
...
def _load_monthly_docs() -> list[dict[str, Any]]:
    db = _require_db()
    docs = list(db[MONTHLY_COLLECTION].find().sort("month_date", -1))
    if not docs:
        raise RuntimeError("monthly_fund_data 데이터가 없습니다. 먼저 일별/월별 집계를 실행하세요.")
    return _apply_running_total_principal(docs)


def _get_month_group_key(date_str: str) -> tuple[int, int]:
    dt = datetime.datetime.strptime(date_str, "%Y-%m-%d").date()
    return dt.year, dt.month


def _build_monthly_doc_from_group(year: int, month: int, docs: list[dict[str, Any]], memo: str) -> dict[str, Any]:
    ...
    now = _get_now_kst()
    # 종료일을 '데이터가 있는 마지막 날'이 아니라 '해당 월의 마지막 영업일'로 고정
    month_date = _get_last_trading_day_of_month(year, month)
...
def _build_monthly_docs_from_daily() -> list[dict[str, Any]]:
    daily_docs = load_daily_docs_for_aggregation()
    grouped: dict[tuple[int, int], list[dict[str, Any]]] = defaultdict(list)
    for doc in daily_docs:
        grouped[_get_month_group_key(str(doc["date"]))].append(doc)

    db = _require_db()
    # 메모를 (year, month) 튜플을 키로 하여 로드 (영업일 변경 대비)
    existing_memo_by_month = {}
    for doc in db[MONTHLY_COLLECTION].find({}, {"month_date": 1, "memo": 1}):
        month_date = str(doc["month_date"])
        month_key = _get_month_group_key(month_date)
        existing_memo_by_month[month_key] = str(doc.get("memo", "") or "")

    monthly_docs: list[dict[str, Any]] = []
    for (year, month), group_docs in grouped.items():
        memo = existing_memo_by_month.get((year, month), "")
        monthly_docs.append(_build_monthly_doc_from_group(year, month, group_docs, memo))

    return sorted(monthly_docs, key=lambda item: str(item["month_date"]))


def load_monthly_table_data() -> dict[str, Any]:
    monthly_docs = _load_monthly_docs()
    active_month_date = str(monthly_docs[0]["month_date"]) if monthly_docs else ""
    return {
        "active_month_date": active_month_date,
        "rows": _build_api_rows(monthly_docs),
...
def aggregate_active_month_data() -> dict[str, str]:
    db = _require_db()
    monthly_docs = _build_monthly_docs_from_daily()
    if not monthly_docs:
        raise RuntimeError("월별 집계에 사용할 daily_fund_data 데이터가 없습니다.")

    valid_month_dates = {str(doc["month_date"]) for doc in monthly_docs}
    db[MONTHLY_COLLECTION].delete_many({"month_date": {"$nin": list(valid_month_dates)}})
    for doc in monthly_docs:
        db[MONTHLY_COLLECTION].update_one(
            {"month_date": doc["month_date"]},
            {"$set": doc},
            upsert=True,
        )
    return {"month_date": str(monthly_docs[-1]["month_date"])}


def update_monthly_row(month_date: str, payload: dict[str, Any]) -> dict[str, str]:
    target_month_date = str(month_date or "").strip()
    if not target_month_date:
        raise RuntimeError("수정할 월을 찾을 수 없습니다.")

    invalid_keys = [key for key in payload if key not in {"month_date", "memo"}]
    if invalid_keys:
        raise RuntimeError("월별에서는 비고만 수정할 수 있습니다.")

    db = _require_db()
    result = db[MONTHLY_COLLECTION].update_one(
        {"month_date": target_month_date},
        {"$set": {"memo": str(payload.get("memo", "") or "").strip(), "updated_at": _get_now_kst()}},
    )
    if result.matched_count == 0:
        raise RuntimeError("수정할 월별 데이터를 찾지 못했습니다.")
    return {"month_date": target_month_date}
