...
def calculate_rsi_for_ticker(
    close_prices: pd.Series,
    period: int | None = None,
    ema_smoothing: float | None = None,
) -> float:
    """
    ...
    Args:
        close_prices: 종가 시리즈
        period: RSI 기간 (None이면 config 사용)
        ema_smoothing: EMA 평활화 계수 (None이면 config 사용)
    ...
    # 설정 로드 (인자 우선, 없으면 config)
    rsi_period = period if period is not None else RSI_CALCULATION_CONFIG.get("period", 14)
    rsi_ema_smoothing = ema_smoothing if ema_smoothing is not None else RSI_CALCULATION_CONFIG.get("ema_smoothing", 2.0)

    # 데이터 길이 확인 (RSI period + 추가 여유분)
    min_required = rsi_period + 5
