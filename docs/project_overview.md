...
*   **📊 계좌별 순위 화면**: 이동평균 고정 추세선과 종목풀별 `SHORT_MA_DAYS`/`LONG_MA_DAYS`를 기준으로 종목 이격과 기울기를 즉시 확인합니다.
*   **🟩 실제 보유 강조**: 실제 보유 종목을 녹색 행으로 표시합니다.
*   **📈 Steady Momentum (`/strategy-sm`)**: 선택한 종목풀에서 장기 이평선 이격 상위 종목을 월 단위로 선정·유지합니다. 이평선 일수는 전략 전용 값이고 설정은 풀별로 저장되며, 같은 규칙의 월간 백테스트(빈 슬롯 현금, 수익인출전략 비교 포함)를 함께 제공합니다.
