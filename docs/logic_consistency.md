...
3. **데이터 충분성 검증**
   ...
   - **이상적인 데이터 요구량** (충분한 데이터가 있는 경우) - *(예: MA 90일 기준)*:
     - TEMA/EMA/DEMA: `MA기간 × 2.0` (180일)
     - HMA: `MA기간 × 1.5` (135일)
     - SMA/WMA: `MA기간 × 1.0` (90일)
   - **완화된 데이터 요구량** (데이터가 부족한 신규 ETF) - *(예: MA 90일 기준)*:
     - TEMA/EMA/DEMA: `MA기간 × 1.0` (90일) - multiplier의 절반
     - HMA: `MA기간 × 1.2` (108일) - 최소 1.2배 (구현 제약)
     - SMA/WMA: `MA기간 × 1.0` (90일) - 최소 1배 유지
   - 완화된 기준도 충족하지 못하면 제외
...
- **예외:** **손절(CUT_STOPLOSS)** 은 쿨다운을 무시하고 즉시 매도 가능
- **정의:**
  - `COOLDOWN_DAYS=0`: "즉시 재진입 가능" (단, 당일 매수 후 당일 매도는 불가. 최소 1일 보유)
  - `COOLDOWN_DAYS=1`: "1일 대기" (오늘 거래 시 내일 거래 불가, 모레 가능)

| 항목 | 추천 | 백테스트 |
|------|------|---------|
| 파일 | `portfolio.py` | `portfolio_runner.py` |
| 매수 쿨다운 | 134-154줄 (`<=` 사용) | 190줄, 1120줄 (`+1` 추가) |
| 매도 쿨다운 | 156-173줄 (`<=` 사용) | 400-401줄 (`+1` 추가) |

**동작:**
1. **매수 쿨다운:** `last_sell_date` 확인 → `COOLDOWN_DAYS` 이내면 매수 불가 (WAIT)
2. **매도 쿨다운:** `last_buy_date` 확인 → `COOLDOWN_DAYS` 이내면 매도 불가 (HOLD)
3. **손절 예외:** 수익률이 `STOP_LOSS_PCT` 이하로 떨어지면 쿨다운 무시하고 `CUT_STOPLOSS`
...
**쿨다운 로직 수정 시:**
- [ ] `portfolio.py` 134-173줄 수정 (매수/매도 쿨다운 모두 확인)
- [ ] `portfolio_runner.py` 190줄, 400줄, 1120줄 동일하게 수정
- [ ] **손절(CUT_STOPLOSS)** 은 매도 쿨다운 예외 처리 확인
