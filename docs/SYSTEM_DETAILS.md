...
- `zsettings/stocks/<국가>.json`에 정의된 모든 `recommend_enabled` ETF를 후보군으로 사용합니다.
- **벤치마크 ETF**는 후보군에 포함되지 않으며, 시장 상황 모니터링 및 성과 비교 목적으로만 사용됩니다.
- 추천·백테스트·튜닝·캐시 갱신이 모두 동일한 티커 목록을 사용합니다.
...
- **설정:**
  - `0`일 설정 시: 즉시 재진입 가능 (단, 당일 매수 후 당일 매도는 불가. 최소 1일 보유)
  - `1`일 설정 시: 하루 대기 (오늘 거래 → 내일 불가 → 모레 가능)
- 잦은 매매로 인한 수수료 손실 방지 및 추세 지속성 확보
...
### 튜닝 결과 반영 로직 (Top 3 Ensemble)

과적합(Overfitting)을 방지하고 일반화된 성능을 확보하기 위해, 단일 최고점(Top 1)이 아닌 **상위 3개 조합의 앙상블** 결과를 사용합니다.

1. **MA_PERIOD (이동평균 기간)**
   - 상위 3개 조합의 **평균값(Average)**을 반올림하여 사용합니다.
   - *이유: 기간값은 연속적인 성격이 강하므로 평균이 안정적입니다.*

2. **기타 파라미터 (TOPN, 손절, RSI 등)**
   - 상위 3개 조합의 **최빈값(Mode)**을 사용합니다.
   - 만약 3개의 값이 모두 다르면, **1위 조합의 값**을 우선적으로 사용합니다.
   - *이유: 이산적인 설정값들이므로 평균보다는 빈도가 높은 값이 신뢰도가 높습니다.*



---

## 8. 핵심 보유 종목 (Core Holdings)
...
계정 설정 파일 (예: `zsettings/account/k1.json`)에서 `CORE_HOLDINGS` 파라미터를 사용합니다.
