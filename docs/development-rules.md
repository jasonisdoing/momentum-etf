...
2. **종목 가격의 소스**
    ...
    - **필수 기간의 가격이 캐시에 없으면** 추천/백테스트/튜닝/캐시갱신/메타 업데이트 등 어떤 스크립트든 즉시 `MissingPriceDataError`를 발생시키고 중단한다. 자동 재시도나 침묵 상태에서의 제외 처리는 허용하지 않는다.
    - 상장일(`listing_date`) 이후의 데이터만 존재하는 종목은 상장일을 시작점으로 간주하여 검증하며, 상장 이전 구간이 비어 있다는 이유로 누락 처리해서는 안 된다.
    - 거래일(`trading_calendar`) 정보 역시 실행 전에 프리패치하여 공유하며, 런타임 도중 `get_trading_days()` 호출로 부족 구간을 보충하지 않는다. 거래일을 준비하지 못하면 즉시 실패로 종료한다.
