...
- `zsettings/stocks/<국가>.json`에 정의된 모든 `recommend_enabled` ETF를 후보군으로 사용한다.
- **벤치마크 ETF**는 후보군에 포함되지 않으며, 시장 모니터링용으로만 사용된다.
- 추천·백테스트·튜닝·가격 캐시 갱신이 모두 동일한 티커 목록을 사용해 결과가 일관된다.
...
- **쿨다운 기간**: 매수/매도 후 일정 기간 동안 반대 매매 금지 (단, 손절은 예외)
