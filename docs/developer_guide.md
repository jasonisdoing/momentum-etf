...
*   `services/`: **외부 API/데이터 연동 통합 계층**
    ...
    *   `etf_holdings_service.py`: 한국 ETF 구성종목 비중 Mongo 캐시 조회/저장 및 KRX 로그인 세션 기반 수집. KRX 원본 PDF의 `COMPST_ISU_CD`를 `raw_code`로 저장하고, 해외 구성종목은 가능하면 Yahoo 검색으로 `ticker/name`을 정규화합니다.
