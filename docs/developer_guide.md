...
2.  **자산 상태 반영**: 추천은 단순한 시그널 목록이 아니라, 해당 계좌의 **현재 현금 비중과 보유 종목 수**를 고려하여 생성됩니다. 현재 구조에서는 계좌 종목 전체를 계속 보유하고, 버킷별 상승 점수 합계로 버킷 목표 비중을 계산한 뒤 버킷 내부 종목에는 균등 비중을 적용합니다. 필요할 때만 종목별 `비중/타겟비중`과 예정 비중조절 문구를 표시합니다.
