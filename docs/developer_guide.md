...
*   `logic/backtest/`: 핵심 시뮬레이션 로직 (계좌 상태 관리, 매매 실행, 수익률 계산 등)
*   `logic/tune/`: 파라미터 최적화 로직
*   `logic/common/`: 백테스트 엔진이 사용하는 핵심 로직 (시그널, 필터링, 포트폴리오 관리)
...
1.  **로직 수정**: `logic/backtest/` 또는 `logic/common/` 수정
2.  **검증**:
...
3.  **현실적 제약 준수**: `portfolio_topn` (최대 보유 종목 수)과 `cash_mode` (시장 방어 모드) 등 모든 전략적 제약 조건이 반영된 최종 결과입니다.
