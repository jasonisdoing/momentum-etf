...
| **SELL_TRAILING** | 매도. 고점 대비 하락폭이 설정된 비율(`TRAILING_STOP_PCT`)을 초과함. |
...
*   보유 기간 중 최고가(Highest Price)를 추적합니다.
*   현재 가격이 최고가 대비 `TRAILING_STOP_PCT` 이상 하락하면 **이익 실현** 또는 **손실 제한**을 위해 매도합니다.
*   이는 상승 추세를 최대한 따라가면서, 추세가 꺾일 때 빠르게 빠져나오기 위함입니다.
...
### (5) 교체 매매 (Replacement) 로직
*   보유 종목보다 훨씬 더 강력한 상승 추세를 보이는 새로운 종목이 나타나면 교체 매매를 수행합니다.
*   **조건**: `신규 종목 점수 > 보유 종목 점수 + REPLACE_SCORE_THRESHOLD`
*   **순차적 탐색 (Iterative Search)**:
    1.  먼저 동일한 카테고리의 보유 종목이 있다면 우선적으로 비교합니다.
    2.  동일 카테고리가 없다면, 보유 종목 중 **점수가 가장 낮은 종목부터 순서대로** 비교합니다.
    3.  이때, 비교 대상 종목이 **쿨다운(Cooldown)** 상태라면 교체할 수 없으므로 건너뛰고 **다음으로 점수가 낮은 종목**을 탐색합니다.
    4.  즉, 가장 약한 종목이 쿨다운이라도 포기하지 않고, 그 다음 약한 종목을 찾아 교체 기회를 모색합니다.

## 4. 매매 가격 기준 (Execution Price)

시스템은 백테스트와 추천 모두 일관된 매매 가격 기준을 가지고 있습니다.
...
*   `TRAILING_STOP_PCT`: 트레일링 스탑 비율 (예: 10.0%)
