...
계좌 설정의 `country_code`는 `kor` 또는 `au`만 사용합니다. `tax`는 종목풀 운영 식별자입니다.
...
계좌(`zaccounts/*`)는 계좌 종목 전체를 유니버스로 사용하고, 버킷과 버킷 내부 종목을 모두 균등 분배하는 리밸런싱 방식입니다.
...
2. 계좌에 포함된 활성 버킷 수 `B`를 기준으로 각 버킷 비중을 `1/B`로 계산한다
3. 각 버킷 내부 종목 수 `N`을 기준으로 종목 비중을 `1/B/N`으로 계산한다
4. 점수는 비중 계산이 아니라 정렬/표시용 지표로만 사용한다
