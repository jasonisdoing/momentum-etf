...
이 문서는 현재 시스템의 의사결정 로직(계좌 비중 조절형 리밸런싱)을 설명합니다.
...
계좌(`zaccounts/*`)는 계좌 종목 전체를 유니버스로 사용하고, 점수 0 이상인 종목 중 전체 상위 `TOPN`을 동일 비중으로 편입하는 리밸런싱 방식입니다.
...
2. 점수 0 이상인 종목만 편입 후보로 본다
3. 전체 후보 중 점수 상위 `TOPN` 종목을 선택한다
4. 선택된 종목에는 슬롯당 동일 비중 `1/TOPN`을 부여한다
5. 양수 종목 수가 `TOPN`보다 적으면 남는 슬롯 비중은 현금으로 유지한다
6. 리밸런싱 시점마다 동일 규칙으로 목표 비중을 다시 계산하고 재조정
