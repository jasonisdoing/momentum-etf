"""토스증권 차트(c-chart) 연동 서비스.

/live-24h 의 개별주 카드·미국 가격 캐시 보강용. 비공식 API 이므로 스키마 변경에 취약하다
— 실패 시 예외를 그대로 올리고, 호출부가 카드 단위로 격리 처리한다.

    - 캔들: wts-info-api /api/v2/c-chart/us-s/{code}/{interval}?count=N
        (min:15 × 96 = 최근 24시간, 최신→과거 순으로 반환됨)
"""
...
from config import TOSS_INVEST_API_BASE_URL, TOSS_INVEST_HEADERS
