...
# 업종으로 쓸 대분류 — **앞에 있을수록 우선**한다(좁은 분류 → 넓은 분류).
# 여기 없는 대분류는 업종이 아니라서 쓰지 않는다: 배율(레버리지·인버스)·투자국가·
# (주식)지수·투자전략·배당·단일종목·국내운용사·채권·원자재·통화·부동산·멀티에셋.
_INDUSTRY_LARGE_CATEGORIES: tuple[str, ...] = (
    "0701",  # 트렌드 — 원자력·조선·건설·화장품·데이터센터 … 가장 구체적
    "0601",  # 혁신기술 — AI·K-반도체·2차전지·로봇·방산 …
    "0401",  # (주식)섹터 — 소재·IT·헬스케어 … GICS 계열, 가장 거칠다
)
