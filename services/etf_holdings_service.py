...
def _create_naver_session() -> requests.Session:
    ...
    이전엔 호출마다 새 Session 을 만들어 매 종목 fetch 때 TCP/TLS handshake 가
    누적되었다. 모듈 전역 Session 으로 connection pool 을 공유한다.
    """
...
def fetch_korean_stock_price_snapshot(tickers: list[str], as_of_date: str) -> dict[str, dict[str, Any]]:
    ...
    if target_index == 0:
        ...
        raise RuntimeError(
            f"전일 거래일을 계산할 수 없습니다: as_of_date={normalized_date}, trading_days={normalized_trading_days}"
        )
