...
`status.py`에서 여러 종목의 시세 데이터를 조회할 때, `yfinance` API의 안정성 문제로 인해 반드시 **순차적으로 처리**해야 합니다. 병렬로 처리할 경우 API 요청이 실패하거나 데이터가 오염될 수 있습니다. 이 문제는 과거에 발생하여 의도적으로 순차 처리로 변경된 사항이므로, 향후 코드 수정 시 이 부분을 반드시 고려해야 합니다.
