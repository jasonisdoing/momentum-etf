...
- `web_app.py`: Streamlit 기반 웹 UI. 오늘의 시그널을 시각적으로 보여줍니다.
...
웹 브라우저를 통해 오늘의 시그널을 시각적으로 확인하고, 거래 내역, 종목 등 데이터를 관리합니다.

    python run.py

2) 오늘/다음 거래일 시그널 조회 (CLI)

과거 시뮬레이션 없이 “현재 보유 + 오늘 신호”를 바탕으로 다음 거래일에 대한 시그널을 제안합니다.

    python cli.py <국가코드> --status

예: `python cli.py coin --status`
...
장 마감 이후 자동으로 시그널을 계산하고(교체매매 신호 포함) 슬랙(Slack)으로 알림을 보낼 수 있습니다.
...
- 국가별 전략 파라미터(웹앱 각 국가 탭 → 설정):
  ...
  - 각 국가별로 DB에 저장되어 해당 국가 시그널/백테스트에 반영됩니다.

주의/제약
--------

- signals.py는 pandas_market_calendars를 이용하여 거래일 판정을 수행합니다. 캘린더 업데이트 지연 시 휴장 판단이 달라질 수 있습니다.
- signals.py 경량 모드는 “내일 할 일” 계산에 특화되어 있습니다.
- signals.py는 장중 실행 시, 네이버 금융 웹 스크레이핑을 통해 실시간 가격을 가져오려고 시도합니다. 이 기능은 네이버 웹사이트 구조 변경 시 동작하지 않을 수 있으며, 비공식적인 방법이므로 안정성을 보장하지 않습니다.
