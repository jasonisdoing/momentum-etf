...
def compute_net_trade_note(
    *,
    ...
    is_fully_sold = (
        current_shares_now <= COIN_ZERO_THRESHOLD
        if country == "coin"
        else current_shares_now <= 0.0
    )
    ...
    was_held_before = (
        (prev_shares_estimated > COIN_ZERO_THRESHOLD) or (prev_shares_cached > COIN_ZERO_THRESHOLD)
        if country == "coin"
        else (prev_shares_estimated > 0.0 or prev_shares_cached > 0.0)
    )
