...
def run_nasdaq_switching_notification() -> None:
    ...
    try:
        ...
        print(f"[INFO] 적용 파라미터: {tr.get('defense_ticker')} / Buy {tr.get('drawdown_buy_cutoff')}% / Sell {tr.get('drawdown_sell_cutoff')}%")
    ...
    if table_lines:
        table_text = "\n".join(table_lines)
        message_blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"*=== 추천 목록 ===*\n{table_text}"}})
        message_blocks.append({"type": "divider"})
