"""이동평균선 크로스 전략 백테스트 엔진.

지수 종가가 이동평균(N일) **위면 레버리지 티커**, **아래면 방어 티커(현금 또는 종목)**를 보유한다.
...
# 튜닝 sweep 기본 후보(이동평균 일수). 과적합 경계: '최적 1개'가 아니라 이 분포 전체를 보여줘
# 특정 값만 튀는지 vs 넓게 완만한지(강건성)를 판단하게 한다.
DEFAULT_MA_CANDIDATES: tuple[int, ...] = (5, 10, 20, 40, 60, 90, 120, 150, 200, 240)
...
def run_ma_cross(
    ...
    *,
    ...
    """이동평균선 크로스 전략을 일별로 시뮬레이션해 지표를 반환한다.
    ...
    ``eval_start`` 가 있으면 이동평균은 전체(워밍업 포함) 구간으로 계산하되 **성과는 그 날짜
    이후만** 집계한다 — 이동평균(장기)도 창 시작부터 신호가 살아 있어 기간 비교가 공정해진다.
    반환: ``{ma_days, cumulative_pct, cagr_pct, mdd_pct, sortino, switches, days, leverage_days}`` 또는 데이터 부족 시 None.
    """
    ...
    sma = calculate_moving_average(df["index"], ma_days, min_periods=ma_days)
    valid = sma.notna()
    df = df[valid]
    sma = sma[valid]
    if len(df) < 5:
    ...
    above_ma = df["index"] > sma
    if peak_drawdown_pct is None:
        want_leverage_by_index = above_ma
    else:
        peak_drawdown = (df["index"] / df["index"].cummax() - 1.0) * 100.0
        want_leverage_by_index = above_ma & (peak_drawdown >= -float(peak_drawdown_pct))
...
def run_buy_hold(close: pd.Series, *, eval_start: pd.Timestamp | None = None) -> dict[str, object] | None:
    ...
    이동평균선 크로스 전략과 비교 기준을 맞추기 위해 일별 수익률 곡선으로 MDD/소르티노를 계산한다.
...
def tune_ma_cross(
    ...
    *,
    buy_pct: float,
    sell_pct: float,
    candidates: Sequence[int] = DEFAULT_MA_CANDIDATES,
    peak_drawdown_candidates: Sequence[float],
    eval_start: pd.Timestamp | None = None,
) -> list[dict[str, object]]:
    """이동평균 후보와 고점대비 후보를 sweep 해 소르티노, CAGR 내림차순으로 반환한다.

    '최적 1개'만 강조하지 않는다 — 전체 후보의 소르티노/수익/MDD 를 나란히 반환해
    특정 이동평균 값만 튀는지(과적합 위험) vs 넓게 완만한지(강건)를 화면에서 판단하게 한다.
...
def current_index_judgment(
    ...
    ``gap_pct`` 는 (종가/이동평균-1)% 이고, ``peak_drawdown_pct`` 는 전고점 대비 현재 하락률(%).
    데이터가 이동평균 계산에 부족하면 None.

    이동평균 종류는 config(SMA/EMA). 백테스트(run_ma_cross)와 동일하게 최신 봉의 이동평균 값을 쓴다.
    """
    close = index_close.dropna().sort_index()
    if len(close) < ma_days:
        return None
    ma_value = float(calculate_moving_average(close, ma_days, min_periods=ma_days).iloc[-1])
    if ma_value == 0.0:
        return None
